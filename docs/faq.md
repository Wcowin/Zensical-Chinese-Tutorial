...
!!! info "当前版本状态"
    ...
    | 功能 | 状态 | 说明 |
    |------|------|------|
    | 核心构建 | ✅ 可用 | Rust 运行时 (ZRX)，支持增量构建与并发处理 |
    | 主题系统 | ✅ 可用 | Modern/Classic 双主题 |
    | Markdown 扩展 | ✅ 可用 | 完整的 Python Markdown + PyMdown 支持 |
    | 即时导航 | ✅ 可用 | 无刷新页面切换，支持预加载 |
    | 搜索功能 | ✅ 可用 | 内置客户端搜索，无需配置 |
    | 多语言 (i18n) | ✅ 可用 | 支持 60+ 语言界面本地化 |
    | 博客系统 | ✅ 可用 | 内置博客插件，支持分类、标签、草稿、阅读时间 |
    | RSS 订阅 | ✅ 可用 | 内置 RSS 插件，自动生成订阅源 |
    | 标签系统 | ✅ 可用 | 基础标签功能已支持 |
    | Studio 编辑器 | ✅ 可用 | 专属写作环境，实时链接检测与智能提示 |
    | Disco 搜索引擎 | 🔜 规划中 | 支持倒排索引、向量查询和联邦搜索 |
    | 模块系统 | 🔜 规划中 | 替代传统插件，可组合可扩展的架构 |
    | UI 组件框架 | 🔜 规划中 | 替代模板系统，独立组件自包含渲染与交互 |
    | 嵌套项目 | 🔜 规划中 | 多层级目录、跨项目链接与统一搜索 |
    | 版本管理 | 🔜 规划中 | 基于 Git 或文件夹的发布追踪 |
    
    完整路线图请查看 [about.md](about.md#路线图与愿景) 和 [Zensical Roadmap](https://zensical.org/about/roadmap/)。
...
Zensical 内置博客插件，只需在 `zensical.toml` 中配置即可：
